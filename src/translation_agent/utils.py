...
def get_completion(
    ...
    if model_provider == "deepseek":
        # ic("requesting deepseek")
        client = openai.OpenAI(api_key=os.getenv("DEEPSEEK_API_KEY"),base_url="https://api.deepseek.com")
        return get_completion_deeps(prompt, system_message)
    elif model_provider == "ollama":
        return get_completion_ollama(prompt, system_message)
    else:
        client = openai.OpenAI(api_key=os.getenv("OPENAI_API_KEY"))
        return get_completion_openai(prompt, system_message)
...
{target_lang}:"""
    ...
    translation = get_completion(model_provider, prompt, system_message=system_message)
...
Output only the suggestions and nothing else."""
    ...
    reflection = get_completion(model_provider, prompt, system_message=system_message)
    return reflection
...
Output only the new translation and nothing else."""
    ...
    translation_2 = get_completion(model_provider, prompt, system_message)

    return translation_2


def one_chunk_translate_text(
    source_lang: str, target_lang: str, source_text: str, country: str = "", model_provider: str = "deepseek"
) -> str:
    """
    ...
    translation_1 = one_chunk_initial_translation(
        source_lang, target_lang, source_text, model_provider
    )

    reflection = one_chunk_reflect_on_translation(
        source_lang, target_lang, source_text, model_provider, translation_1, country
    )
    translation_2 = one_chunk_improve_translation(
        source_lang, target_lang, source_text, model_provider, translation_1, reflection
    )
...
def multichunk_initial_translation(
    source_lang: str, target_lang: str, source_text_chunks: List[str], model_provider: str
) -> List[str]:
    """
...
Output only the new translation of the indicated part and nothing else."""
    ...
    for i in range(len(source_text_chunks)):
        ...
        translation_2 = get_completion(model_provider, prompt, system_message=system_message)
...
def multichunk_translation(
    source_lang, target_lang, source_text_chunks, country: str = "", model_provider: str = "deepseek"
):
    """
    ...
    translation_1_chunks = multichunk_initial_translation(
        source_lang, target_lang, source_text_chunks, model_provider
    )
    ic("translation_1_chunks done")
...
def translate(
    ...
    if num_tokens_in_text < max_tokens:
        ...
        final_translation = one_chunk_translate_text(
            source_lang, target_lang, source_text, country, model_provider
        )
        ...
        ic(f"Text split into chunks done. chunk size: {len(source_text_chunks)}")

        translation_2_chunks = multichunk_translation(
            source_lang, target_lang, source_text_chunks, country, model_provider
        )
